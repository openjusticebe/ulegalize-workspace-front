...
import { generateReportEtat } from '../../../services/DossierService';
...
const ReportEtatHonoraireGenerator = ( { showMessage, label, affaireId } ) => {
    ...
    useEffect( () => {
        (async () => {
            ...
            let result;
                result= await generateReportEtat( accessToken, affaireId );
                if ( !result.error ) {
                    let pdf = b64toBlob( result.data, '' );
                    setFile( pdf );
                } else {
                    showMessage( label.affaire.alert103, 'danger' );
                }
...
export default ReportEtatHonoraireGenerator;